...
var configInstance FormatParams

func init() {
	configInstance = FormatParams{}
}

func (FormatParams) Parse(file string) error {
	fileContent, _ := utils.GetSmallFileContent(file)
	var err error = nil
	switch Info.GetExt() {
	case ".yaml", ".yml":
		err = yaml.Unmarshal(fileContent, &configInstance)
	case ".json":
		err = json.Unmarshal(fileContent, &configInstance)
	case ".bson":
	// 空着先
	case ".xml":
		err = xml.Unmarshal(fileContent, &configInstance)
	}
	if err != nil {
		return err
	}
...
func (FormatParams) ParamsToConfig() map[string]any {
	return utils.GetParams(configInstance, "yaml")
}
